...
deflitmus vijay1 := Fence; W.sys_rel X = 1; W.sys_rel Y = 1 || Fence; W.sys_rel Z = 1; W.sys_rel A = 1 || R Y // 1; R Z // 0 || R A // 1; R X // 0
 where sys := { {T0, T1}. PTX, {T2, T3}. x86}
 expect ✓

deflitmus vijay2 := Fence; W.sys_rel X = 1; W.sys_rel Y = 1 || Fence; W.sys_rel Z = 2; W.sys_rel A = 1 || R Y // 1; W Z = 1; R Z // 2 || R A // 1; R X // 0
 where sys := { {T0, T1}. PTX, {T2, T3}. x86}
 expect ✓
